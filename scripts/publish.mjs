...
// 🗣️ THE LANGUAGE GATES LEFT THIS FILE ON 2026-09-13, AND THEY ARE NOT COMING BACK HERE.
// Until that date this publisher carried four of its own -- an acronym refusal with per-entry
// expansions, a wiki-path/forecast-id refusal, a private-vocabulary warning and a revision-language
// warning -- and the weekly-training-plan skill carried a prose checklist saying the same things.
// Two copies drift, and the copy nobody runs drifts first. Calvin ruled: "this new plain language
// guard should be the single source of truth of this guard." That source is
//
//     hermes-training-wiki/tools/plain.py        (`--rules` prints every rule and its origin)
//
// which reads the same artifact this file reads, plus the calendar drafts this file never sees,
// and runs in the wiki's guard workflow on every published/<stem>.json a push adds -- the record
// that reaches this app on merge. So every string that gets here has already been read by it.
// What STAYS here is the app's own shape contract, which nothing else can know: markup (this app
// renders with textContent), field lengths, words per session, and coverage.
//
// ⚠️ Do not re-add a language rule here, however small. Add it to the wiki's ACRONYMS or RULES
// table, where the tests and the CI step are.
